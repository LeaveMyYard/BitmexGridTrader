...
class CurrentOrdersModule(IQDockWidgetModule):
    ...
    def add_order(self, price: float, volume: float) -> int:
        ...
        self.table.setItem(len(self._order_dict), 0, QtWidgets.QTableWidgetItem(str(order_id)))
        for i, value in enumerate(self._order_dict[order_id].to_dict().values()):            
            self.table.setItem(len(self._order_dict), i + 1, QtWidgets.QTableWidgetItem(str(value)))
...
class Order:
    ...
    def to_dict(self):
        return dict(time=self.date, side=self.side, price=self.price, volume=self.volume)